...
Das Core-Schema definiert die primäre XML-Struktur einer <xref:System.Windows.Annotations.Annotation> .  Die Mehrzahl der XML-Elemente, die im Kern Schema definiert sind, entsprechen den Typen im- <xref:System.Windows.Annotations> Namespace.  Das Core-Schema macht drei Erweiterungspunkte verfügbar, wo die Anwendungen eigene XML-Daten hinzufügen können.  Diese Erweiterungs Punkte enthalten <xref:System.Windows.Annotations.Annotation.Authors%2A> , <xref:System.Windows.Annotations.ContentLocatorPart> und "Inhalt".  (Inhaltselemente werden in Form einer Liste bereitgestellt <xref:System.Xml.XmlElement> .)

Das in diesem Thema beschriebene Basis Schema definiert die Erweiterungen für die <xref:System.Windows.Annotations.Annotation.Authors%2A> Inhaltstypen, und, die in <xref:System.Windows.Annotations.ContentLocatorPart> der Initial Windows Presentation Foundation (WPF)-Version enthalten sind.

<a name="CoreSchema"></a>

## <a name="annotations-xml-core-schema"></a>Annotations XML-Kernschema

Das Annotations XML-Kern Schema definiert die XML-Struktur, die zum Speichern von Objekten verwendet wird <xref:System.Windows.Annotations.Annotation> .
...
Das Basis Schema definiert die XML-Struktur für die drei abstrakten Elemente, die im Kern Schema – <xref:System.Windows.Annotations.Annotation.Authors%2A> , und definiert sind <xref:System.Windows.Annotations.ContentLocatorPart> <xref:System.Windows.Annotations.AnnotationResource.Contents%2A> .
