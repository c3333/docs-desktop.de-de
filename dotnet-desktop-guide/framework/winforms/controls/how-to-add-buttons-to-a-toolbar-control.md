...
> Obwohl das <xref:System.Windows.Forms.ToolStrip>-Steuerelement das <xref:System.Windows.Forms.ToolBar>-Steuerelement ersetzt und funktionell erweitert, wird das <xref:System.Windows.Forms.ToolBar>-Steuerelement sowohl aus Gründen der Abwärtskompatibilität als auch, falls gewünscht, für die zukünftige Verwendung beibehalten.  
 ...
 In den folgenden Beispielen wird davon ausgegangen, dass ein- <xref:System.Windows.Forms.ToolBar> Steuerelement zu einem Windows Form hinzugefügt wurde ( `Form1` ).  
  
### <a name="to-add-buttons-programmatically"></a>So fügen Sie Schaltflächen Programm gesteuert hinzu  
  
1. Erstellen Sie in einer Prozedur Symbolleisten Schaltflächen, indem Sie Sie der Auflistung hinzufügen <xref:System.Windows.Forms.ToolBar.Buttons%2A?displayProperty=nameWithType> .  
  
2. Geben Sie Eigenschaften Einstellungen für eine einzelne Schaltfläche an, indem Sie den Index der Schaltfläche über die- <xref:System.Windows.Forms.ToolBar.Buttons%2A> Eigenschaft übergeben.  
  
     Im folgenden Beispiel wird davon ausgegangen, dass ein Formular <xref:System.Windows.Forms.ToolBar> bereits hinzugefügt wurde.  
  
    > [!NOTE]
    > Die Auflistung <xref:System.Windows.Forms.ToolBar.Buttons%2A?displayProperty=nameWithType> ist eine Null basierte Auflistung, sodass der Code entsprechend fortgesetzt werden sollte.  
