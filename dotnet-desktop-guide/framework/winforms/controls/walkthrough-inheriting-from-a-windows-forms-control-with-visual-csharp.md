...
6. Suchen Sie die `InitializeComponent` -Methode, und entfernen Sie die Zeile, die die Eigenschaft zuweist <xref:System.Windows.Forms.ContainerControl.AutoScaleMode%2A> . Diese Eigenschaft ist im Steuerelement nicht vorhanden <xref:System.Windows.Forms.Button> .

7. Klicken Sie im Menü **Datei** auf **Alles speichern**, um das Projekt zu speichern.

    > [!NOTE]
    > Ein visueller Designer ist nicht mehr verfügbar. Da das <xref:System.Windows.Forms.Button> Steuerelement seine eigene Zeichnung durchführt, können Sie seine Darstellung im Designer nicht ändern. Die visuelle Darstellung ist identisch mit der der Klasse, von der Sie erbt (d <xref:System.Windows.Forms.Button> . h.), es sei denn, Sie wird im Code geändert. Sie können trotzdem Komponenten, die über keine Benutzeroberflächenelemente verfügen, zur Entwurfsoberfläche hinzufügen.
...
6. Doppelklicken Sie auf der Registerkarte **alle Windows Forms** der **Toolbox** auf **Bezeichnung** , um dem Formular ein Steuerelement hinzuzufügen <xref:System.Windows.Forms.Label> .
